...
def objectiveMxl(
    ...
    for t in np.arange(nTakes):
        ...
        if nRndUc > 0 and nTakes > 1:
            drawsUcTake = drawsUc[ab, :]
        else:
            drawsUcTake = drawsUc  
        if nRndCo > 0 and nTakes > 1:
...
if __name__ == "__main__":
    ...
    xRndCo = np.array(xRnd) #np.stack((const2, const3), axis = 1)
    xRndUc = np.zeros((0,0)) # #-np.hstack((cost, he, tt))
    xFix = np.array(xFix) #np.stack((cost, he, tt), axis = 1)
