...
if __name__ == "__main__":
    ...
    K = 3 #no. of random parameters
    
    true_alpha = np.array([-0.8, 0.8, 1.2])
    true_zeta = np.array([-0.8, 0.8, 1.0, -0.8, 1.5]) / 2
    ...
    xRnd = np.random.rand(NTJ, K) * 2

    betaInd_tmp = true_zeta[:K] + \
    (np.linalg.cholesky(true_Omega[:K,:K]) @ np.random.randn(K, N)).T
    ...
    print(results['psi'] / omega) 
    print(np.cov(betaInd_tmp.T))