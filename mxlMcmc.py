...
from mxl import corrcov, prepareData, mvnlpdf, probMxl, pPredMxl, transFix, transRnd
...
def estimate(
    ...
    for i in np.arange(simDraws_star):
        ...
        lPInd = probMxl(
                ...
                xRnd, xRnd_transBool, xRnd_trans, nRnd,
                nInd, rowsPerInd, map_obs_to_ind, map_avail_to_obs)
        pIndSim += np.exp(lPInd)
    
    pIndSim /= simDraws_star
...
def mcmcChainPredB(
    ...
    for i in sampleIdx:
        ...
        for t in np.arange(nTakes):
            if nRnd:
                paramRnd = zeta_tmp.reshape((1,nRnd)) + (ch_tmp @ np.random.randn(nRnd, nInd * nSim)).T
...
def mcmcChainPredW(
        chainID, seed,
        mcmc_iterSampleThin, mcmc_iterSampleThinPred, mcmc_disp,
        modelName,
        xFix, nFix, 
        xRnd, nRnd,  
        nInd, nObs, nRow,
